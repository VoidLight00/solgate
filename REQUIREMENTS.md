...
>
> solgate = gpt-5.6 sol/terra/luna(실창 372k input) 각각 위에 **가상 1M 컨텍스트**를 만드는 로컬 프록시.
...
```
Claude Code(sol-1m / terra-1m / luna-1m, 라벨 [1m])
  → CCR :3456 (Anthropic→OpenAI 변환)
  → solgate :8321 (이 프로젝트 — 압축 계층)
  → profile physical base: gpt-5.6-sol / terra / luna
...
| SR2 | `/v1/models`에 가상 모델 `gpt-5.6-{sol,terra,luna}-1m` 3종(context_length 1000000)을 중복 없이 주입하고 upstream 목록 보존 | service_gate.sh |
| SR3 | 추정 토큰 ≤ COMPACT_TRIGGER(300k) 요청은 messages 무변형 패스스루 | unit_gate.sh |
| SR4 | 세 가상 1M profile의 추정 토큰 > 300k 요청은 자동 압축: 오래된 구간을 ~40k 청크로 나누고 base와 다른 물리 summary 후보로 롤링 요약, 최근 ~200k 원문 유지. summary에는 virtual ID와 base self-call 금지 | unit_gate.sh + e2e_compact_gate.sh |
...
| VIRTUAL_CONTEXT | 1000000 | 세 가상 profile의 공개 컨텍스트 선언 |
| SUMMARY_POLICY | sol→terra/luna, terra→luna/sol, luna→terra/sol | base self-call·virtual recursion 방지 |
