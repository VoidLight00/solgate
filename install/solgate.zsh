...
_solgate_cap_for() {
  case "$1" in
    gpt-5.6-sol-1m) print -r -- "1m" ;;   # 가상 1M — solgate가 압축 소유
...
vgpt() {
  ...
  if [[ -n "$1" && "$1" != -* ]]; then
    model="$(_solgate_model_id "$1")" || { print -u2 -- "vgpt: unknown model '$1' (sol/terra/luna/gpt5.5/1m)"; return 2; }
    shift
  else
    model="gpt-5.6-sol"
  fi
...
solgate GPT models ([Nk] = auto-compact 시점 선언, 하드정지 아님):
  ...
  vgpt 1m / vgpt1m→ gpt-5.6-sol-1m[1m]   가상 1M (300k 초과분 롤링 요약, 무손실 아님)
세션 중 전환: /model solgate,gpt-5.6-terra[330k] 등 ([330k] 라벨 필수)
